...
## About
This demonstrates how to scale a consumer service based on the number of messages generated with three different methods. The first two ways use a intermediary resource to consume messages from a kafka topic and forwards the messages via HTTP to the service. OpenShift Serverless is utilized to scale up the service based on load. The third method uses the Kubernetes Horizontal Pod Autoscaler (HPA) but simplifies the configuration by using a joint MS-Red Hat upstream project called KEDA. With KEDA we can scale up a non-HTTP service that consumes directly from the kafka topic based on the number of records on the topic.
